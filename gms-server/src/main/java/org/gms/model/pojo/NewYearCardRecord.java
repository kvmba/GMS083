...
public class NewYearCardRecord {
    ...
    public static void printNewYearRecords(Character chr) {
        ...
        for (NewYearCardRecord nyc : chr.getNewYearRecords()) {
            ...
            chr.dropMessage(5, "发件人名称: " + nyc.senderName);
            chr.dropMessage(5, "发件人丢弃: " + nyc.senderDiscardCard);

            chr.dropMessage(5, "收件人ID: " + nyc.receiverId);
            chr.dropMessage(5, "收件人名称: " + nyc.receiverName);
            chr.dropMessage(5, "收件人丢弃: " + nyc.receiverDiscardCard);
            chr.dropMessage(5, "已接收: " + nyc.receiverReceivedCard);

            chr.dropMessage(5, "留言: " + nyc.message);
            ...
            if (send) {
                if (nyc.senderId == cid) {
                    ...
                    if (other != null && other.isLoggedInWorld()) {
                        ...
                        other.dropMessage(6, "[新年] " + chr.getName() + " 扔掉了新年贺卡。");
                    }
                }
            } else {
