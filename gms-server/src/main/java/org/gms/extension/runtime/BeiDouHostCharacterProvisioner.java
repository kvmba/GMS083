...
public final class BeiDouHostCharacterProvisioner implements HostCharacterProvisioner {
    ...
    public HostCharacterProvisionResult provision(HostCharacterProvisionRequest request) throws Exception {
        return provisionInternal(request, null);
    }
    ...
    public HostCharacterProvisionResult provision(
        ...
        return provisionInternal(request, callback);
    }

    private HostCharacterProvisionResult provisionInternal(
            HostCharacterProvisionRequest request,
            HostCharacterMetadataCallback callback
    ) throws Exception {
        ...
        Provisioned provisioned = transactions.execute(connection -> {
            ...
            HostCharacterProvisionResult result = new HostCharacterProvisionResult(
                    character.getId(), accountId, request.characterName());

            // Extension work shares this connection and therefore this transaction:
            // a callback failure rolls the native rows back with it.
            if (callback != null) {
                callback.persist(connection, new HostCharacterProvisionMetadata(
                        character.getId(), accountId,
                        request.accountName(), request.characterName(), request.worldId()));
            }

            return new Provisioned(character, result);
        });
