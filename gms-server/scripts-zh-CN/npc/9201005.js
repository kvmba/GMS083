...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (!weddingIndoors) {
            ...
            if (status == 0) {
                var text = "欢迎来到#b大教堂#k！有什么可以帮你的吗？";
                var choice = ["怎么准备婚礼？", "我订婚了，想安排婚礼", "我想参加婚礼"];
                                ...
                                if (cserv.isWeddingReserved(weddingId)) {    // registration check
                                    ...
                                    if (hasCommon || hasPremium) {
                                        ...
                                        if (resStatus > 0) {
                                            ...
                                            partner.dropMessage(6, "婚礼助理：你们都收到了15张婚礼门票。请柬只能在婚礼开始前发出。您的 " + wedType + " 婚礼将从 " + placeTime + " 开始。穿好衣服，别迟到！");

                                            if (!hasSuitForWedding(player)) {
                                                player.dropMessage(5, "婚礼助理：请在出席婚礼前购买一件婚纱。可以在最左边的婚礼村婚庆店购买。");
                                            }

                                            if (!hasSuitForWedding(partner)) {
                                                partner.dropMessage(5, "婚礼助理：请在出席婚礼前购买一件婚纱。可以在最左边的婚礼村婚庆店购买。");
                                            }
                                        } else {
